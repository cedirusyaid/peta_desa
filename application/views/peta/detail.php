...
<script>
    ...
    // Function to parse WKT polygon (Google Maps version)
    ...
    function filterMarkersByCategory(categoryId) {
        ...
        // Automatically open info window if only one marker is visible
        if (visibleMarkers === 1 && lastVisibleMarker) {
            google.maps.event.trigger(lastVisibleMarker, 'click');
        }
    ...
    function initMap() {
        ...
        <?php foreach ($lokasi as $loc): ?>
            ...
            var markerIcon = {
                <?php if(file_exists(FCPATH.'images/icons/'.$loc->kategori_icon)): ?>
                    url: '<?= base_url("images/icons/".$loc->kategori_icon) ?>',
...
<script src="https://maps.googleapis.com/maps/api/js?key=<?=MAP_API?>&callback=initMap" async defer></script>
